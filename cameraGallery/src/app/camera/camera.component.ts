...
export class CameraComponent implements AfterViewInit {
	constructor(private senaService: SenasService) {}
	...
	eliminarFoto(idx: number) {
		var ImagenesActuales = this.captures.filter(
			(item) => item !== this.captures[idx]
		);
	...
	getBase64StringFromDataURL(captures: string[]) {
		for (let i = 0; i < captures.length; i++) {
			this.imagenesProcedas.push(
				captures[i].replace('data:', '').replace(/^.+,/, '')
			);
		}
	}
	...
	procesar() {
		if (this.captures.length <= 0) {
			...
			this.senaService
				.crearSena(this.imagenesProcedas, this.inputText)
				.subscribe((res) => {
					console.log(res);
					Swal.fire({
						icon: 'success',
